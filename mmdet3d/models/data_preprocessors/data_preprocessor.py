...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        # pass through optional custom dense features for multimodal models
        if 'sam_feat' in inputs:
            batch_inputs['sam_feat'] = self._stack_dense_feature_batch(
                inputs['sam_feat'])
        if 'sam_freq_feat' in inputs:
            batch_inputs['sam_freq_feat'] = self._stack_dense_feature_batch(
                inputs['sam_freq_feat'])

        return {'inputs': batch_inputs, 'data_samples': data_samples}

    @staticmethod
    def _stack_dense_feature_batch(
            features: Union[Tensor, Sequence[Tensor]]) -> Union[Tensor, Sequence[Tensor]]:
        """Stack dense feature tensors when pseudo-collate leaves them as a list."""
        if isinstance(features, torch.Tensor):
            return features
        if not is_seq_of(features, torch.Tensor) or len(features) == 0:
            return features

        first_shape = features[0].shape
        if any(feat.shape != first_shape for feat in features[1:]):
            return features

        first = features[0]
        if first.dim() == 4 and first.size(0) == 1:
            return torch.cat(list(features), dim=0)
        if first.dim() in (2, 3):
            return torch.stack(list(features), dim=0)
        return features
