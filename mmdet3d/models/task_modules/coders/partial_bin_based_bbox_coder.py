...
class PartialBinBasedBBoxCoder(BaseBBoxCoder):
    ...
    def encode(self, gt_bboxes_3d: BaseInstance3DBoxes,
            ...
            (Added DEBUG checks to catch index out of bounds)
            """
            
            # ================== 【强力调试代码 (修正版)】 ==================
            # 将数据移到 CPU
            labels_cpu = gt_labels_3d.detach().cpu()
            max_label = labels_cpu.max().item() if len(labels_cpu) > 0 else -1
            min_label = labels_cpu.min().item() if len(labels_cpu) > 0 else 0
            
            # [修改点]：mean_sizes 是 list，使用 len() 获取类别数
            num_defined_classes = len(self.mean_sizes)
            
            # 检查是否越界
            if max_label >= num_defined_classes or min_label < 0:
                import sys
                import numpy as np # 确保导入 numpy
                print("\n" + "="*50)
                print("🛑 [CRITICAL ERROR CAUGHT] 发现非法类别标签！")
                print(f"👉 预设类别数 (len(mean_sizes)): {num_defined_classes}")
                print(f"👉 当前 Batch 中发现的最大标签: {max_label}")
                print(f"👉 当前 Batch 中发现的最小标签: {min_label}")
                
                # 找出具体的非法值
                invalid_mask = (labels_cpu >= num_defined_classes) | (labels_cpu < 0)
                invalid_values = labels_cpu[invalid_mask].unique()
                print(f"👉 具体的非法标签值列表: {invalid_values.numpy()}")
                
                print("💡 原因分析: 数据集中包含了未定义的类别，或者 .pkl 索引文件未经过滤。")
                print("="*50 + "\n")
                
                # 强制抛出异常
                sys.stdout.flush()
                raise ValueError(f"Label {max_label} is out of bounds for {num_defined_classes} classes.")
            # ================== 【调试代码结束】 ==================
            ...
            # 注意：这里原代码会自动处理 list 到 tensor 的转换，不需要动
