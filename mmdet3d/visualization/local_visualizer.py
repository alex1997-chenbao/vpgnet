...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def draw_proj_bboxes_3d(
        ...
        # 获取保存路径（从 input_meta 中提取文件名，如果没有则用时间戳或默认名）
        # SUN RGB-D 通常在 input_meta['sample_idx'] 存储 ID
        sample_idx = input_meta.get('sample_idx', 'unknown_sample')
        save_dir = 'work_dirs/pred_results_txt'
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        
        file_path = os.path.join(save_dir, f'{sample_idx}.txt')

        # 提取 3D 框数据 (x, y, z, l, w, h, yaw)
        # 如果你之前在 predict 阶段过滤了分数，这里的 tensor 就是过滤后的结果
        boxes_np = bboxes_3d.tensor.cpu().numpy()
        classes = getattr(self, 'dataset_meta', {}).get('classes', None)
        ...
        with open(file_path, 'w') as f:
            for i, box in enumerate(boxes_np):
                ...
                # 如果有类别名则用类别名，否则用数字 ID
                class_name = classes[label_idx] if classes is not None else str(label_idx)
                
                # 写入文件：类别 x y z l w h yaw
                line = f"{class_name} {box[0]:.4f} {box[1]:.4f} {box[2]:.4f} " \
    ...
    def _draw_instances_3d(self,
        ...
        if vis_task in ['mono_det', 'multi-modality_det']:
            ...
            if isinstance(img, list) or (isinstance(img, (np.ndarray, Tensor))
                ...
                self.draw_proj_bboxes_3d(
                    bboxes_3d, input_meta,labels=labels_3d, edge_colors=colors)
