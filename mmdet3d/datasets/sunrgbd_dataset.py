...
class SUNRGBDDataset(Det3DDataset):
    ...
    def __init__(self,
                 ...
                 sam_feat_prefix: Optional[str] = None,
                 sam_freq_feat_prefix: Optional[str] = None,
                 ...
                 **kwargs) -> None:
        self.sam_feat_prefix = sam_feat_prefix
        self.sam_freq_feat_prefix = sam_freq_feat_prefix
        super().__init__(
            data_root=data_root,
            ann_file=ann_file,
            metainfo=metainfo,
        ...
        if self.modality['use_camera']:
            ...
            if self.default_cam_key is not None:
                ...
                info['depth2img'] = np.array(
                    ...
                    dtype=np.float32)
                _, img_ext = osp.splitext(info['img_path'])
                feat_filename = osp.basename(info['img_path']).replace(
                    img_ext, '.pt')
                if self.sam_feat_prefix is not None:
                    info['sam_feat_path'] = osp.join(self.sam_feat_prefix,
                                                     feat_filename)
                else:
                    info['sam_feat_path'] = info['img_path'].replace(
                        '/image/', '/sam_f/').replace(img_ext, '.pt')
                if self.sam_freq_feat_prefix is not None:
                    info['sam_freq_feat_path'] = osp.join(
                        self.sam_freq_feat_prefix, feat_filename)
                else:
                    info['sam_freq_feat_path'] = info['img_path'].replace(
                        '/image/', '/sam_freq_f/').replace(img_ext, '.pt')
        # print(info['depth2img'].shape)
