...
def _load_torch_pt_cpu(path: str):
    """Load a torch serialized tensor from disk with fast fallbacks."""
    load_variants = (
...
@TRANSFORMS.register_module()
class LoadSamFeature:
    """Load SAM feature from .pt file based on the prepared path.

    The path is expected to be in data_info['sam_feat_path'].
    """

    def __init__(self,
                 key: str = 'sam_feat_path',
                 output_key: str = 'sam_feat',
                 use_mmap: bool = True,
                 tensor_key: Optional[str] = None):
        # key: info 字典中存储特征路径的键 (即 'sam_feat_path')
        self.key = key
        # output_key: 特征加载后存储在 info 字典中的键 (即 'sam_feat')
        self.output_key = output_key
        self.use_mmap = use_mmap
        self.tensor_key = tensor_key

    def _extract_tensor(self, payload, feat_path: str):
        """Support both raw tensors and dict payloads such as DINOv3 exports."""
        if torch.is_tensor(payload):
            return payload
        if not isinstance(payload, dict):
            raise TypeError(
                f'Unsupported feature payload type {type(payload)} from {feat_path}')

        quant_info = payload.get('quantization', None)
        if isinstance(quant_info, dict) and quant_info.get('dtype') == 'uint8':
            quant_keys = []
            if self.tensor_key is not None:
                quant_keys.append(self.tensor_key)
            quant_keys.extend([
                self.output_key, 'sam_feat', 'feature', 'features',
                'fpn_0', 'fpn_1', 'fpn_2', 'data'
            ])
            seen = set()
            quantized = None
            ...
            if quantized is None:
                tensor_keys = [
                    key for key, value in payload.items() if torch.is_tensor(value)]
                raise KeyError(
                    f'No quantized tensor found in {feat_path}. '
                    f'available_tensor_keys={tensor_keys}')

            scale = float(quant_info.get('scale', 1.0))
            offset = float(quant_info.get('min', 0.0))
            dequant_dtype = quant_info.get('dequant_dtype', 'float16')
            dtype = torch.float32 if dequant_dtype == 'float32' else torch.float16
            return quantized.to(torch.float32).mul_(scale).add_(offset).to(dtype)

        candidate_keys = []
        if self.tensor_key is not None:
            candidate_keys.append(self.tensor_key)
        candidate_keys.extend([
            self.output_key, 'patch_features', 'sam_feat',
            'fpn_0', 'fpn_1', 'fpn_2'
        ])
        ...
        raise KeyError(
            f'No tensor feature found in {feat_path}. tensor_key={self.tensor_key}, '
            f'available_tensor_keys={tensor_keys}')

    def __call__(self, data_info: dict) -> dict:
        """Call function to load SAM features."""
        
        feat_path = data_info.get(self.key, None)
        
        if feat_path is None:
            # 如果路径不存在，可能需要返回 None 或报错，取决于您的需求
            print(f"Warning: SAM feature path '{self.key}' not found in data_info.")
            return data_info

        try:
            if self.use_mmap:
                sam_feat = _load_torch_pt_cpu(feat_path)
            else:
                try:
                    sam_feat = torch.load(
                        feat_path, map_location='cpu', weights_only=True)
                except TypeError:
                    sam_feat = torch.load(feat_path, map_location='cpu')
            sam_feat = self._extract_tensor(sam_feat, feat_path)
            
            # 将加载的数据存储到 data_info 中
            # 如果需要 NumPy 数组，可以加一步转换：
            # data_info[self.output_key] = sam_feat.cpu().numpy()
            
            data_info[self.output_key] = sam_feat
            
        except Exception as e:
            # 如果文件损坏或不存在，可以捕获错误并返回 None，让 DataLoader 重新获取样本
            print(f"Error loading SAM feature from {feat_path}: {e}")
            return None 
    ...
    def __repr__(self):
        return (f'{self.__class__.__name__}('
                f'key={self.key}, output_key={self.output_key}, '
                f'use_mmap={self.use_mmap}, tensor_key={self.tensor_key})')
                
                
                
@TRANSFORMS.register_module()
class LoadFrequencyFeature:
    """Load pre-computed frequency feature (X_task) from a .pt file.

    The path is expected to be prepared in the Dataset and stored 
    in data_info['sam_freq_feat_path'].
    """

    def __init__(self, 
                 key: str = 'sam_freq_feat_path',  # <--- 更改默认的路径键
                 output_key: str = 'sam_freq_feat',
                 use_mmap: bool = True): # <--- 更改输出键
        
        # key: info 字典中存储频率特征路径的键
        self.key = key
        # output_key: 特征加载后存储在 data_info 中的键
        self.output_key = output_key
        self.use_mmap = use_mmap

    def __call__(self, data_info: dict) -> Optional[dict]:
        """Call function to load frequency features."""
        
        feat_path = data_info.get(self.key, None)
        
        if feat_path is None:
            print(f"Warning: Frequency feature path '{self.key}' not found in data_info.")
            # 如果路径信息缺失，返回原始数据
            return data_info

        try:
            if self.use_mmap:
                frequency_feat = _load_torch_pt_cpu(feat_path)
            else:
                try:
                    frequency_feat = torch.load(
                        feat_path, map_location='cpu', weights_only=True)
                except TypeError:
                    frequency_feat = torch.load(feat_path, map_location='cpu')
            
            # 将加载的数据存储到 data_info 中
            data_info[self.output_key] = frequency_feat
            
        except Exception as e:
            # 捕获文件不存在或损坏的错误
            print(f"Error loading frequency feature from {feat_path}: {e}")
            return None  # 返回 None 允许 DataLoader 重新采样
